...
| **P2** ✅ | **`auto`** | Done: §4.1 (system) + §4.2 (last complete turn) + §4.3 (lookback fillers, ~15-block spacing under the 4-cap) placement + §4.5 model-minimum gate + OpenAI no-op (`Available(auto)`=true, `Apply(auto)` places); savings metric + zero-hit alert scaffold (PR #199). Placement engine complete. **Remaining: per-route auto enable** (needs a dashboard-be `prompt_cache` block). | Measured savings > write premium |
